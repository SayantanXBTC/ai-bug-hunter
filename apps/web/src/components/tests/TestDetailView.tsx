    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-neutral-500">
              ...
              {app && (
                <span>
                  in <span className="font-medium text-neutral-700">{app.name}</span>
                </span>
              )}
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {canWrite && (
              <button
                ...
                onClick={() => void del()}
                disabled={deleting}
                className="inline-flex items-center gap-1.5 rounded border border-red-300 bg-white px-3 py-1.5 text-sm font-medium text-red-700 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-400 disabled:opacity-60"
              >
                <IconTrash size={12} />
      ...
      {tc.description && (
        <Section title="Objective">
          <p className="text-sm text-neutral-700">{tc.description}</p>
        </Section>
      )}
      ...
      <Section
        ...
        action={
          <button
            ...
            className="inline-flex items-center gap-1 rounded border border-neutral-300 bg-white px-2.5 py-1 text-xs font-medium text-neutral-700 hover:bg-neutral-50"
          >
      ...
      >
        {runs === null ? (
          <div className="py-4 text-xs text-neutral-500">Loading runs…</div>
        ) : runs.length === 0 ? (
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-neutral-200 text-left text-[11px] font-medium uppercase tracking-wider text-neutral-500">
            ...
            <tbody className="divide-y divide-neutral-100">
              {runs.map((r) => (
                <tr
                  key={r.id}
                  className={onNavigateToRun ? 'cursor-pointer hover:bg-neutral-50' : ''}
      ...
      <Section title="Reliability">
        {reliability === null ? (
          <div className="py-4 text-xs text-neutral-500">Loading…</div>
        ) : reliability === 'none' ? (
...
function BackBar({ onBack }: { onBack: () => void }): JSX.Element {
  return (
    <button
      type="button"
      onClick={onBack}
      className="inline-flex items-center gap-1.5 rounded border border-neutral-300 bg-white px-2.5 py-1 text-xs font-medium text-neutral-700 hover:bg-neutral-50 focus:outline-none focus:ring-2 focus:ring-neutral-400"
    >
...
function StepRow({ step, index }: { step: TestActionRecord; index: number }): JSX.Element {
  ...
  return (
    <li className="flex items-start gap-3 py-2">
      <span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-neutral-100 text-[10px] font-medium tabular-nums text-neutral-600">
        {index}
      </span>
      <div className="min-w-0 flex-1">
        <div className="text-sm font-medium text-neutral-900">{label}</div>
        <div className="mt-0.5 space-y-0.5 text-xs text-neutral-600">
          {step.action === 'navigate' && typeof step.url === 'string' && (
            <div>
              URL: <span className="font-mono text-neutral-800">{step.url}</span>
            </div>
          )}
          {typeof step.selector === 'string' && (
            <div>
              Target: <span className="font-mono text-neutral-800">{step.selector}</span>
            </div>
          )}
          ...
          {step.action === 'selectOption' && typeof step.value === 'string' && (
            <div>
              Option: <span className="font-mono text-neutral-800">{step.value}</span>
            </div>
          )}
          {step.action === 'press' && typeof step.key === 'string' && (
            <div>
              Key: <span className="font-mono text-neutral-800">{step.key}</span>
            </div>
          )}
          {step.action === 'wait' && typeof step.durationMs === 'number' && (
            <div>
              Duration: <span className="tabular-nums text-neutral-800">{step.durationMs}ms</span>
            </div>
          )}
          {step.action === 'waitForSelector' && typeof step.timeoutMs === 'number' && (
            <div>
              Timeout: <span className="tabular-nums text-neutral-800">{step.timeoutMs}ms</span>
            </div>
          )}
        </div>
      </div>
...
function ReliabilityPill({ status }: { status: string }): JSX.Element {
  const map: Record<string, string> = {
    ...
    insufficient_data: 'border-neutral-200 bg-neutral-50 text-neutral-600',
  };
