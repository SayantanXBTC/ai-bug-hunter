...
export function TestRunList({ onSelect }: Props): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <button
            type="button"
            onClick={refresh}
            className="inline-flex items-center gap-2 rounded-md border border-[var(--border)] bg-[var(--surface-hover)] px-3 py-1.5 text-xs font-medium text-[var(--text)] transition-colors hover:border-violet-500/40 hover:bg-[var(--surface-hover)] focus:outline-none focus:ring-2 focus:ring-violet-500/40"
          >
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-glass)] p-4 backdrop-blur-md">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <input
            ...
            className="flex-1 rounded-md border border-[var(--border)] bg-[var(--surface-elevated)] px-3 py-1.5 text-sm text-[var(--text)] placeholder:text-[var(--text-subtle)] focus:border-violet-500/50 focus:outline-none focus:ring-1 focus:ring-violet-500/40"
          />
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)}
            className="rounded-md border border-[var(--border)] bg-[var(--surface-elevated)] px-3 py-1.5 text-sm text-[var(--text)] focus:border-violet-500/50 focus:outline-none focus:ring-1 focus:ring-violet-500/40"
          >
        ...
        <div className="mt-4 overflow-x-auto">
          {error ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[10px] font-medium uppercase tracking-[0.2em] text-[var(--text-subtle)]">
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr
                    ...
                    className="tr-row group cursor-pointer border-t border-[var(--border)] transition-colors hover:bg-[var(--surface-hover)] focus:bg-[var(--surface-hover)] focus:outline-none"
                  >
                    ...
                    <td className="px-3 py-3 font-mono text-xs text-[var(--text-subtle)]">
                      {r.id.slice(0, 8)}
                    </td>
                    <td className="px-3 py-3 text-right text-[var(--text-subtle)] group-hover:text-violet-300">
          ...
          <div className="mt-4 flex items-center justify-between border-t border-[var(--border)] pt-3 text-xs text-[var(--text-subtle)]">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="rounded border border-[var(--border)] px-2 py-1 text-[var(--text-muted)] hover:bg-[var(--surface-hover)] disabled:opacity-30"
              >
                Prev
              </button>
              <button
